...
def create_availability_pie_chart(df: pd.DataFrame, on_col: str, ready_col: Optional[str], date_range: tuple) -> tuple[go.Figure, dict]:
    ...
    # Convert dates to pandas datetime for comparison and add full day for end_date
    if hasattr(start_date, 'total_seconds'):  # datetime.timedelta
        ...
        # Add 1 day to include the full end date (end of day)
    ...
    # Filter data to only include records within the selected date range
    df_filtered = df.copy()
    if 'Date' in df_filtered.columns:
        ...
        mask = (df_filtered['Date'] >= start_pd) & (df_filtered['Date'] <= end_pd)
    ...
    # Calculate time NOT COVERED by our tool
    uncovered_minutes = total_period_minutes - minutes_covered_by_tool
    ...
    # Add period information below chart - ENHANCED WITH ACTUAL DATA RANGE
    ...
    # ADD ACTUAL DATA RANGE INFORMATION (FROM ALL FILES, NOT FILTERED)
    actual_data_range_text = ""
    if len(df) > 0 and 'Date' in df.columns:
        # Use original df (not df_filtered) to get COMPLETE range from first to last file
        df_dates = df.copy()
        ...
        actual_data_range_text = f"Actual Data Range: {actual_start.strftime('%Y-%m-%d %H:%M')} — {actual_end.strftime('%Y-%m-%d %H:%M')} ({actual_duration_hours:.1f}h)"
    ...
    if 'Date' in df.columns and len(df) > 0:
        # Create date range for the COMPLETE document range (not filtered range)
        df_complete = df.copy()
        df_complete['Date'] = pd.to_datetime(df_complete['Date'])
        actual_start = df_complete['Date'].min()
        actual_end = df_complete['Date'].max()
        full_date_range = pd.date_range(start=actual_start.date(), end=actual_end.date(), freq='D')
        
        # Get unique dates that have data from the COMPLETE document
        data_dates = pd.to_datetime(df_complete['Date'].dt.date).unique()
...
def show_metrics_analysis(df_processed, available_flags):
    ...
    if on_col:
        ...
        **📊 Data Coverage Details:**
        ...
        • **No Data Coverage:** {len(missing_dates)} days without data
        """)
        ...
        if missing_dates:
            if len(missing_dates) <= 10:
                ...
                st.warning(f"**📅 Specific dates with no data:** {missing_dates_str}")
            else:
                missing_dates_str = ", ".join(missing_dates[:10])
                st.warning(f"**📅 Dates with no data:** {missing_dates_str}... and {len(missing_dates)-10} more dates")
    else:
...
def generate_complete_html_report(df_display, available_flags, detected_client, custom_title, date_range_main):
    ...
    if available_flags and 'Date' in df_display.columns and not df_display['Date'].dropna().empty:
        # Create the timeline chart using the same function as dashboard
    ...
    # SECTION 3: GLOBAL OPERATION DISTRIBUTION
    if on_col and metrics:
        efficiency_fig = create_efficiency_donut_v2(metrics, on_col, ready_col)
        charts_html.append(f"""
        <div class="section-container">
        <h2 style="text-align: center; color: #2C3E50; margin-bottom: 2rem;">Global Operation Distribution</h2>
        <div class="chart-wrapper">{efficiency_fig.to_html(full_html=False, include_plotlyjs=False)}</div>
        </div>
        """)
    ...
    if on_col:
        ...
        <div class="info-box">
            ...
            <p>• <strong>No Data Coverage:</strong> {len(missing_dates)} days without data</p>
        </div>
        </div>
        """)
    
    # SECTION 5: SYSTEM STATES TIME SERIES
    if on_col and 'Date' in df_display.columns and not df_display['Date'].dropna().empty:
    ...
    # SECTION 6: ADVANCED SYSTEM ANALYSIS
    charts_html.append("""
    <div class="section-container">
    ...
    <body>
        ...
        <div class="footer">
            <p><strong>OPTIBAT Monthly Report</strong> - Professional Analysis Tool</p>
            <p>Developed by Juan Cruz E. | Powered by Optimitive</p>
            <p>Report generated with {len(df_display):,} data records</p>
        </div>
