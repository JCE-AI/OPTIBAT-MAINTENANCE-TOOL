...
def create_availability_pie_chart(df: pd.DataFrame, on_col: str, ready_col: Optional[str], date_range: tuple) -> tuple[go.Figure, dict]:
    ...
    if 'Date' in df.columns and len(df_filtered) > 0:
        # Create date range for the entire analysis period
        start_pd = pd.to_datetime(start_date)
        end_pd = pd.to_datetime(end_date)
        full_date_range = pd.date_range(start=start_pd, end=end_pd, freq='D')
        ...
        # Find missing dates (dates in analysis period but not in data)
        missing_dates = []
