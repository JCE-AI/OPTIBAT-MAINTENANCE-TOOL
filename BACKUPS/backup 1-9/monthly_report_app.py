...
class OptibatMetricsAnalyzer:
    ...
    def create_timeline_chart(df: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            ...
            title={
                'text': "Línea de Tiempo del Sistema (Estados de Flags)", 
    ...
    def create_interactive_duration_chart(df: pd.DataFrame, flag_column: str = 'OPTIBAT_ON') -> go.Figure:
        ...
        if flag_column not in df.columns or df[flag_column].empty:
            ...
            fig.update_layout(height=400, title=f"Estado {flag_column.replace('_', ' ')} - Sin Datos")
            return fig
        ...
        fig.update_layout(
            title=f"Estados {flag_column.replace('_', ' ')} con Duraciones",
            xaxis_title="Fecha",
            yaxis_title="Estado",
            height=500,
            hovermode='x unified',
...
def show_unified_dashboard():
    ...
    with col3:
        ...
        <div style="background: {OPTIMITIVE_COLORS['warning']}; color: white; padding: 1rem; border-radius: 10px; text-align: center;">
            <h4 style="margin: 0;">🗓️ REGISTROS</h4>
...
def show_metrics_analysis(df_processed, available_flags):
    ...
    with kpi_cols[0]:
        status_color = "🟢" if kpis['system_on'] == "Activo" else "🔴" if kpis['system_on'] == "Inactivo" else "⚠️"
        st.metric("Estado Sistema", f"{status_color} {kpis['system_on']}")
    ...
    with kpi_cols[5]:
        heartbeat_color = "🟢" if kpis['heartbeat_status'] == "Normal" else "🔴" if kpis['heartbeat_status'] == "Stuck" else "⚠️"
        st.metric("HeartBeat", f"{heartbeat_color} {kpis['heartbeat_status']}")
    
    # Gauges de flags disponibles - MOSTRAR TODAS LAS FLAGS
    if available_flags:
        st.markdown("### 🎛️ Estado de Flags en Tiempo Real")
    ...
    if 'OPTIBAT_ON' in df_display.columns:
        st.markdown("### ⏱️ Estados OPTIBAT_ON con Duraciones")
    ...
    # Timeline MEJORADO sin superposiciones
    if "Date" in df_display.columns and not df_display["Date"].dropna().empty:
        st.markdown("### 📊 Timeline Mejorado del Sistema")
        timeline_fig = OptibatMetricsAnalyzer.create_enhanced_timeline_chart(df_display)
...
def main():
    ...
    with st.sidebar:
        ...
        if uploaded_files_global:
            ...
            st.success(f"✅ {len(uploaded_files_global)} archivo(s) cargado(s)")
        ...
        # Info compacta
        with st.expander("ℹ️ Sistema de Flags"):
            st.markdown(f"""
