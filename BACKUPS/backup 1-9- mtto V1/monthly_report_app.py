        ...
        if flag in df.columns:
            ...
            if not data.empty:
                ...
                if 'Communication_ECS' in flag:
                    if uptime_pct < 50:
                        observaciones = "⚠️ Baja conectividad ECS"
                    elif cambios > len(data) * 0.5:
                        observaciones = "⚠️ Conexión inestable"
                    else:
...
def create_file_segments_from_data(df):
    """Crea una columna 'File' simulada basada en períodos temporales de los datos"""
    if df.empty:
    ...
    try:
        ...
        # Crear nombres de archivo descriptivos
        segment_names = {}
        for segment_id in df['file_segment'].unique():
            ...
            try:
                if isinstance(segment_id, pd.Timestamp):
                    ...
                    segment_names[segment_id] = f"DATA_{segment_id.strftime('%Y%m%d')}"
                    ...
                    if start_date.date() == end_date.date():
                        segment_names[segment_id] = f"DATA_{start_date.strftime('%Y%m%d')}"
                    else:
                        segment_names[segment_id] = f"DATA_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}"
            except:
                # Fallback si hay problemas con fechas
                segment_names[segment_id] = f"Dataset_Segment_{segment_id}"
...
def create_ready_downs_by_weekday_chart(df, ready_col):
    ...
    # Contar caídas por día de la semana
...
def show_metrics_analysis(df_processed, available_flags):
    ...
    st.markdown("---")
    st.markdown("### Análisis Avanzado del Sistema")
    ...
    if ready_col is None or on_col is None:
        st.warning("⚠️ No se encontraron las columnas necesarias (OPTIBAT_READY y OPTIBAT_ON) para el análisis avanzado.")
        return
    
    # 1. EVOLUCIÓN DEL PORCENTAJE EN OPTIBAT READY EN EL TIEMPO
    st.markdown("#### 1. Evolución del Porcentaje de Tiempo en OPTIBAT_READY=1")
    if "Date" in df_display.columns:
        ready_evolution_fig = create_ready_evolution_chart(df_display, ready_col)
        st.plotly_chart(ready_evolution_fig, use_container_width=True)
    else:
        st.info("📊 Se requiere una columna de fecha para mostrar la evolución temporal.")
    
    # 2. EVOLUCIÓN DEL PORCENTAJE DEL TIEMPO EN LAZO CERRADO
    st.markdown("#### 2. Evolución del Porcentaje del Tiempo en Lazo Cerrado (OPTIBAT_ON=1)")
    if "Date" in df_display.columns:
        closed_loop_evolution_fig = create_closed_loop_evolution_chart(df_display, on_col)
        st.plotly_chart(closed_loop_evolution_fig, use_container_width=True)
    else:
        st.info("📊 Se requiere una columna de fecha para mostrar la evolución temporal.")
    
    # 3. PORCENTAJE DE TIEMPO EN LAZO CERRADO POR ARCHIVO
    st.markdown("#### 3. Porcentaje de Tiempo en Lazo Cerrado por Archivo")
    # Crear columna File simulada basada en períodos temporales
    df_with_file = create_file_segments_from_data(df_display.copy())
    ...
    # 4. OPTIBAT READY Y NO READY POR ARCHIVO
    st.markdown("#### 4. OPTIBAT Ready y No Ready por Archivo")
    ...
    # 5. UTILIZACIÓN EN LAZO ABIERTO Y CERRADO POR ARCHIVO
    st.markdown("#### 5. Utilización en Lazo Abierto y Cerrado por Archivo")
    ...
    # 6. NÚMERO DE CAÍDAS DE OPTIBAT READY POR DÍA DE LA SEMANA
    st.markdown("#### 6. Número de Caídas de OPTIBAT Ready por Día de la Semana")
    if "Date" in df_display.columns:
    ...
    # 7. HORARIOS DE CAÍDA DE OPTIBAT READY
    st.markdown("#### 7. Horarios de Caída de OPTIBAT Ready")
    if "Date" in df_display.columns:
    ...
    # 8. DURACIÓN OPTIBAT READY
    st.markdown("#### 8. Duración de Períodos OPTIBAT Ready")
    if "Date" in df_display.columns:
    ...
    # 1. ANÁLISIS DE PERFORMANCE DE FLAGS
    st.markdown("#### 📊 Análisis de Performance de Flags")
    performance_results = analizar_performance_flags(df_display, available_flags)
    if not performance_results.empty:
        performance_fig = create_performance_chart(performance_results)
        st.plotly_chart(performance_fig, use_container_width=True)
        
        # Mostrar tabla de resultados
        with st.expander("📋 Detalles del Análisis de Performance", expanded=False):
            st.dataframe(performance_results, use_container_width=True)
    else:
        st.info("ℹ️ No hay suficientes datos para el análisis de performance de flags")
    
    # 2. ANÁLISIS GLOBAL DE CAÍDAS FLAG_READY
    if 'Flag_Ready' in df_display.columns:
        st.markdown("#### ⚠️ Análisis Global de Caídas Flag_Ready")
        caidas_analysis = analizar_caidas_flag_ready(df_display)
        if caidas_analysis['total_caidas'] > 0:
            caidas_fig = create_caidas_chart(caidas_analysis)
            st.plotly_chart(caidas_fig, use_container_width=True)
            
            # Estadísticas de caídas
            col1_caidas, col2_caidas, col3_caidas = st.columns(3)
            with col1_caidas:
                st.metric("Total de Caídas", caidas_analysis['total_caidas'])
            with col2_caidas:
                st.metric("Duración Promedio", f"{caidas_analysis['duracion_promedio']:.1f} min")
            with col3_caidas:
                st.metric("Caída Más Larga", f"{caidas_analysis['duracion_maxima']:.1f} min")
        else:
            st.success("✅ No se detectaron caídas de Flag_Ready en el período seleccionado")
    
    # 3. RESUMEN COMPARATIVO POR ARCHIVO
    if len(st.session_state.get('global_txt_files', [])) > 1:
        st.markdown("#### 📋 Resumen Comparativo por Archivo")
        
        # Explicación del análisis
        st.info("""
        **📊 ¿Qué muestra este análisis?**
        
        - **Gráfico Superior (Uptime)**: Porcentaje de tiempo que el sistema estuvo operativo en cada archivo
        - **Gráfico Inferior (Registros)**: Cantidad de datos disponibles por archivo
        
        **🔍 Deducciones posibles:**
        - Archivos con Uptime >95%: Sistema funcionando óptimamente
        - Archivos con Uptime 90-95%: Funcionamiento bueno con incidencias menores
        - Archivos con Uptime <90%: Posibles problemas operativos o mantenimientos
        - Variaciones en registros: Diferentes períodos de medición o pérdidas de datos
        """)
        
        resumen_files = generar_resumen_por_archivo(st.session_state['global_txt_files'], df_display)
        if not resumen_files.empty:
            resumen_fig = create_resumen_files_chart(resumen_files)
            st.plotly_chart(resumen_fig, use_container_width=True)
            
            with st.expander("📊 Tabla Comparativa de Archivos", expanded=False):
                st.dataframe(resumen_files, use_container_width=True)
                st.markdown("""
                **💡 Interpretación de la tabla:**
                - **Registros**: Número total de mediciones en el archivo
                - **Uptime (%)**: Porcentaje de tiempo operativo del sistema
                - **Anomalías**: Número de eventos anómalos detectados
                - **Calidad**: Evaluación general basada en uptime (Excelente >95%, Buena >90%, Regular ≤90%)
                """)
    
    # 4. EVOLUCIÓN DEL SISTEMA EN EL TIEMPO
    if "Date" in df_display.columns and not df_display["Date"].dropna().empty:
        st.markdown("#### 📈 Evolución del Sistema en el Tiempo")
        evolucion_fig = crear_grafico_evolucion_sistema(df_display, available_flags)
        st.plotly_chart(evolucion_fig, use_container_width=True)
        
        # Análisis de tendencias
        tendencias = analizar_tendencias_sistema(df_display, available_flags)
        if tendencias:
            with st.expander("📈 Análisis de Tendencias", expanded=False):
                for flag, tendencia in tendencias.items():
                    if tendencia['significativa']:
                        color = "green" if tendencia['direccion'] == 'mejora' else "red"
                        st.markdown(f"**{flag}**: <span style='color:{color}'>{tendencia['descripcion']}</span>", 
                                  unsafe_allow_html=True)
                    else:
                        st.markdown(f"**{flag}**: {tendencia['descripcion']}")
        else:
            st.info("ℹ️ No hay suficientes datos para análisis de tendencias")
    
    # 5. GRÁFICO DE ROSQUILLA GLOBAL MEJORADO (Ya existe arriba, pero aquí agregamos versión global)
    st.markdown("#### Distribución Global de Estados por Flag")
    rosquilla_global_fig = generar_grafico_rosquilla_global(df_display, available_flags)
    st.plotly_chart(rosquilla_global_fig, use_container_width=True)
    
    # === NUEVOS ANÁLISIS ESPECÍFICOS SOLICITADOS ===
    st.markdown("---")
    st.markdown("### 🔬 Análisis Específicos de Flag_Ready")
    
    # 6. DISTRIBUCIÓN DE DURACIÓN DE CAÍDAS
    if 'Flag_Ready' in df_display.columns:
        st.markdown("#### Distribución de la Duración de las Caídas de Flag_Ready")
        duracion_fig = analizar_duracion_caidas_flag_ready(df_display)
        if duracion_fig.data:
            st.plotly_chart(duracion_fig, use_container_width=True)
        else:
            st.info("ℹ️ No se detectaron caídas para analizar duraciones")
    
    # 7. CAÍDAS POR HORA DEL DÍA
    if 'Flag_Ready' in df_display.columns and 'Date' in df_display.columns:
        st.markdown("#### Número de Caídas de Flag_Ready por Hora del Día")
        hora_fig = analizar_caidas_por_hora(df_display)
        if hora_fig.data:
            st.plotly_chart(hora_fig, use_container_width=True)
            st.info("💡 **Interpretación**: Identifica patrones horarios de caídas - útil para detectar problemas recurrentes en turnos específicos")
        else:
            st.info("ℹ️ No hay datos de fecha suficientes para análisis horario")
    
    # 8. CAÍDAS POR DÍA DE LA SEMANA
    if 'Flag_Ready' in df_display.columns and 'Date' in df_display.columns:
        st.markdown("#### Número de Caídas de Flag_Ready por Día de la Semana")
        dia_fig = analizar_caidas_por_dia_semana(df_display)
        if dia_fig.data:
            st.plotly_chart(dia_fig, use_container_width=True)
            st.info("💡 **Interpretación**: Revela patrones semanales - puede indicar problemas operativos o de mantenimiento en días específicos")
        else:
            st.info("ℹ️ No hay datos de fecha suficientes para análisis semanal")
    
    # === ANÁLISIS POR ARCHIVO ===
    if len(st.session_state.get('global_txt_files', [])) > 1:
        st.markdown("---")
        st.markdown("### 📊 Distribuciones por Archivo")
        
        files_list = st.session_state['global_txt_files']
        
        # 9. DISTRIBUCIÓN TIEMPO OPTIBAT_ON POR ARCHIVO
        if 'OPTIBAT_ON' in df_display.columns:
            st.markdown("#### Distribución de Tiempo OPTIBAT_ON por Archivo")
            optibat_fig = analizar_distribucion_tiempo_por_archivo(df_display, 'OPTIBAT_ON', files_list)
            if optibat_fig.data:
                st.plotly_chart(optibat_fig, use_container_width=True)
        
        # 10. DISTRIBUCIÓN TIEMPO FLAG_READY POR ARCHIVO
        if 'Flag_Ready' in df_display.columns:
            st.markdown("#### Distribución de Tiempo Flag_Ready por Archivo")
            ready_fig = analizar_distribucion_tiempo_por_archivo(df_display, 'Flag_Ready', files_list)
            if ready_fig.data:
                st.plotly_chart(ready_fig, use_container_width=True)
        
        # 11. PORCENTAJE TIEMPO EN LAZO CERRADO POR ARCHIVO
        st.markdown("#### Porcentaje de Tiempo en Lazo Cerrado por Archivo")
        lazo_fig = analizar_lazo_cerrado_por_archivo(df_display, files_list)
        if lazo_fig.data:
            st.plotly_chart(lazo_fig, use_container_width=True)
            st.info("💡 **Interpretación**: Muestra qué archivos tienen mayor tiempo en control automático vs manual")
        else:
            st.info("ℹ️ No se encontró columna específica de lazo cerrado - usando OPTIBAT_ON como referencia")
