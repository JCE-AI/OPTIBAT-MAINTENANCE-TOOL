...
def ts_with_background_regions(df: pd.DataFrame, tcol: str, on_col: str, ready_col: Optional[str]) -> go.Figure:
    """
    ...
    fig.update_layout(
        ...
        xaxis=dict(title="Fecha y Hora", rangeslider=dict(visible=True)), # Slider para zoom
        yaxis=dict(title="Estado", tickvals=[0, 1], ticktext=["OFF", "ON"], range=[-0.1, 1.1]),
        height=400,
...
def show_metrics_analysis(df_processed, available_flags):
    ...
    if on_col:
        ...
        if ready_col and 'Date' in df_display.columns and not df_display['Date'].dropna().empty:
            st.markdown("#### Serie Temporal de Estados del Sistema")
            temporal_states_fig = ts_with_background_regions(df_display, 'Date', on_col, ready_col)
            # Aumentar altura 3 veces más (900px para mejor visualización)
    ...
    if 'OPTIBAT_ON' in df_display.columns:
        ...
        duration_fig = OptibatMetricsAnalyzer.create_interactive_duration_chart(df_display, 'OPTIBAT_ON')
        # Aumentar altura 3 veces más (900px para mejor visualización)
        duration_fig.update_layout(height=900)
        st.plotly_chart(duration_fig, use_container_width=True)
        
        # TABLA RESUMEN DE ON/OFF CON FECHAS
    ...
    st.markdown("#### 1. Evolución Temporal: OPTIBAT_READY y Lazo Cerrado")
