...
def ts_with_background_regions(df: pd.DataFrame, tcol: str, on_col: str, ready_col: Optional[str], show_durations: bool = False) -> go.Figure:
    ...
    # 3. AGREGAR ANOTACIONES DE DURACIÓN SI SE SOLICITA
    if show_durations:
        ...
        for i in range(len(changes) - 1):
            ...
            if duration.total_seconds() > 0:
                ...
                # Agregar anotación con flecha y cuadro
                fig.add_annotation(
                    x=current[tcol] + duration/2,
                    y=current[on_col] + 0.1,
                    text=duration_text,
                    showarrow=True,
                    arrowhead=2,
                    arrowcolor="blue",
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title="Fecha y Hora", 
            rangeslider=dict(visible=False if show_durations else True),  # Sin range-slider si hay duraciones
            fixedrange=show_durations  # Sin wheel-zoom si hay duraciones
        ),
        yaxis=dict(
            title="Estado", 
            tickvals=[0, 1], 
